...
//    [m_ziggeo record];
    ...
    [VideoScreenRecorder.sharedRecorder startRecording];
}
...
//    [m_ziggeo playVideo:Last_Video_Token];
    ...
    [VideoScreenRecorder.sharedRecorder stopRecording];
}
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        dispatch_async(dispatch_get_main_queue(), ^{
            if (currentType == Video) {
                [m_ziggeo playVideo:token];
            } else if (currentType == Audio) {
                [m_ziggeo startAudioPlayer:token];
            } else if (currentType == Image) {
                [m_ziggeo showImage:token];
            }
        });
    });
}
